...
public class Cliente_FisicoDao extends ConexaoDb{
	...
	final String SQL_ALTERA_CLIENTE_FISICO = "UPDATE Cliente_fisico SET  NOME_CLI_F = ?, TEL_CLI_F = ?, CPF_FUNC = ?, EMAIL_CLI_F = ?, CEP_CLI_F = ?  WHERE ID_cli_fisico = ?"; 
	final String SQL_DELETA_CLIENTE_FISICO = "DELETE FROM Cliente_fisico WHERE ID_cli_fisico = ?";
	...
	public int inserir(Cliente_Fisico clientef){
		...
		try (Connection connection = this.conectar();
			...
			pst.setInt(3, clientef.getCpf_func());
	...
	public int alterar(Cliente_Fisico clientef) {
			...
			PreparedStatement pst	= connection.prepareStatement(SQL_ALTERA_CLIENTE_FISICO);)
		{
			...
			pst.setString(4, clientef.getEmail_cli_f());
			pst.setString(5, clientef.getCep_cli_f());
			pst.setInt(6, clientef.getID_cli_fisico());
			quantidade = pst.executeUpdate();			
		} 
		catch (SQLException e) 
		{
	...
	public int deleta(Cliente_Fisico clientef)
	{
		...
		{
			pst.setInt(1, clientef.getID_cli_fisico());
			quantidade = pst.executeUpdate();
